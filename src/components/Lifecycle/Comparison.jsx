...
const Comparison = ({scenarioData, selected, setSelected, update, units, emissions}) => {
  ...
  useEffect(() => {
    ...
    npvsSorted.sort((a, b) => b[0] - a[0]); //sort npv values
    ...
    if (selected){
      ...
      for (let i = 0; i < reordered.length; i++){
        ...
        if (reordered[i].npvTotalValues){
          shortY = emissions ? reordered[i].npvTotalValues : (reordered[i].npvTotalValues.map(i => -i));
          newNPVs.push(shortY.at(-1).toFixed(2));
          
          let year = reordered[i].year0;

          //x value for graph
          let current = {
            x: reordered[i].npvTotalValues.map((_, i) => i+year).slice(0, shortY.length),
            y: shortY,
            type: 'line',
            mode: 'lines+markers',
...
}, [scenarioData, selected, update, emissions, units]);
  ...
  return (
    <div className = {styles.section}>
      ...
      <div className = {styles.compareCols}>
        <CompareGraph data ={data} units = {units} emissions = {emissions}/>
        <CompareTable names = {names} npvs = {npvs} colors = {tableColors} units = {units}/>
      </div>
